...
_MODULES = [
    ("m1","Enterprise AI Talent Matching","Core","1-second talent matching."),
...
MODULES = [
    ...
    for mid, name, cat, desc in _MODULES
]
...
class AIEngine:
    ...
    def chat(cls, msg: str) -> str:
        ...
        if any(w in m for w in ["job","hire","salary","career"]):
            return cls.KB["job"]
        if any(w in m for w in ["money","earn","referral"]):
            return cls.KB["earn"]
...
async def analyze_interview(
    ...
    if len(data) > MAX_UPLOAD_SIZE:
        return JSONResponse({"status": "error", "message": "File too large"}, status_code=413)
    transcript = (
        f"Simulated {language} interview transcript. Candidate shows strong "
        "technical expertise, clear communication, problem-solving. 5+ years AI/ML."
    )
...
# ═══════════════════════════════════════════════════════════════
# PORTFOLIO / SALARY / SKILL GAP / QUIZ / COMPANIES
# ═══════════════════════════════════════════════════════════════
...
def root():
    ...
    return HTMLResponse("""
    <!DOCTYPE html><html><head><title>ERCORS Backend</title>
